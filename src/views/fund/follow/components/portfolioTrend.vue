    ...
    methods: {
        ...
        async getPerformanceTrend() {
            try {
                ...
                let { result } = (await PortfolioReturnTrend(params)) || {}
                result = result || {}
                const { portfolioTrend = {} } = result || {}
                this.list = portfolioTrend.trend
                this.requesed = true
        ...
        initChart() {
            ...
            const chgRate = yData[yData.length - 1]

            const { fallColor, riseColor } = colors
            const baseColor = chgRate < 0 ? fallColor : riseColor
            const riseColors = [
                {
                    offset: 0.7,
                ...
                color: [
                    {
                        ...
                        colorStops: [...(chgRate >= 0 ? riseColors : fallColors)],
