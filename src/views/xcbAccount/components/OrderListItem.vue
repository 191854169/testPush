...
export default {
    ...
    computed: {
        list() {
            return [
                {
                    label: this.$t('detail.subscribeAmount'),
                ...
                { label: this.$t('orderList.date'), value: dateFormat(this.info.statusUpdateTime) },
            ]
        },
    },
    watch: {},
    ...
    mounted() {},
    destroyed() {},
    filters: { formatterFilter, currencyFilter, dateFormat },
    methods: {
        gotoDetails() {
            this.$goPage('/order-detail', { orderNumber: this.info.orderNumber })
        },
    },
}
</script>
