    ...
    <div class="assets">
        ...
        <div class="card">
            <div class="box-content">
                ...
                <div class="asset-summary">
                    <div class="bg">
                        <multi-img class="bg-img" name="icon_liushui_dcb" directory="fixedDepositTreasure"></multi-img>
                    </div>
                    <asset-total
                        class="total"
                    ...
                    <template v-if="showSubscribeBtn">
                        <button class="btn subscribe" @click="goProductList">
                            {{ $t('viewProducts') }}
                        </button>
                    </template>
        ...
        <ul class="list" @click="onListClick">
            <li class="list-item mask" data-key="about">
                <span class="list-item__text">{{ $t('whatIsDepositTreasure') }}</span>
        ...
        <div class="guide">
            ...
            <div class="right" @click="goProductList">{{ $t('productList') }}</div>
        </div>
        <!-- 账户选择组件 -->
...
import { ORDER_STATUS_MAP, PRODUCT_SUBSCRIBE_STATUS_MAP } from '@/views/fixedDepositTreasure/config/common'
import { ftdAssetsSummary, ftdOrderStatistics, ftdProductList, ftdUserHoldings } from '@/apis/ftd'
import { getPageVisibleSupportProperty, getValueFromLocalStorage, isEmpty, floatToRatio, milliFormat } from '@/utils'
...
export default {
    ...
    methods: {
         ...
         */
        registerAppButton() {
            this.$jsBridge?.setButton('right1', { customIcon: require('@/assets/images/common/icon_navi_tips@3x.png') }, () => {
                this.goHelpCenter()
            })
        },
        ...
        async fetchHoldingList() {
            try {
                ...
                let list = []
                const { result } = await ftdUserHoldings(params)
                list = result?.list || []
                const test = false
        ...
        sortByAsc(a, b, key) {
            const aV = a[key]
            const bV = b[key]
            const unsortable = ['actualInterest', 'todayInterest', 'accumInterest'].includes(key)
            const aOS = a.orderStatus === ORDER_STATUS_MAP.keysMap.submitted && unsortable
            const bOS = b.orderStatus === ORDER_STATUS_MAP.keysMap.submitted && unsortable
            // 如果按照升序排列，需要将数据值为''的 放到最后列.
            // 如果两个都为''的情况下,会按照initIndex来排序
            if (!(aV || bV) || (aOS && bOS)) return a.initIndex - b.initIndex
            if (!aV || aOS) return 1
            if (!bV || bOS) return 1
        ...
        async fetchSubscribeRecords() {
            try {
                const { result } = await ftdOrderStatistics()
        ...
        async fetchAssetsSummary() {
            try {
                const { result } = await ftdAssetsSummary()
